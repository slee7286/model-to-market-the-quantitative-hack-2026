...
class ThresholdRecommendationTests(unittest.TestCase):
    ...
    def test_recommender_does_not_lower_entry_threshold_by_default(self) -> None:
        start = datetime(2026, 6, 24, 4, 30, tzinfo=timezone.utc)
        with tempfile.TemporaryDirectory() as tmpdir:
            database = Path(tmpdir) / "trading.db"
            with SQLiteStore(database) as store:
                bars = []
            ...
            recommendation = recommend_thresholds_from_store(
                database,
                target_symbols=("BTC/USD",),
                current_entry_threshold=1.25,
                current_exit_threshold=0.75,
            )

        self.assertTrue(recommendation.available)
        self.assertGreaterEqual(recommendation.recommended_entry_threshold, 1.25)
        self.assertIn("lower ENTRY_THRESHOLD", recommendation.reason)
